...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<object> result = Emps.Join(Depts, emp => emp.Deptno, dept => dept.Deptno, //Join Emps,Depts, then provide keys for joining
                                                  (emp, dept) => new { emp.Ename, emp.Job, dept.Dname }); //define result
            return result;
        }
        ...
        {
            int result = arr.GroupBy(number => number)
                            .Where(g => g.Count() % 2 != 0)
                            .Select(g => g.Key)
                            .Single();
            return result;
        ...
        {
            IEnumerable<Dept> result = Depts.GroupJoin(Emps, d => d.Deptno, e => e.Deptno, (department, departmentEmployees) => new { department, departmentEmployees})
                                            .Where(group => group.departmentEmployees.Count() == 5 || group.departmentEmployees.Count() == 0)
                                            .OrderBy(group => group.department.Dname)
                                            .Select(group => group.department);
            return result;
        }
    }
    ...
    public static class CustomExtensionMethods
    {
        ...
        {
            var result = emps.Where(e => emps.Any(em => em.Mgr != null && em.Mgr.Empno == e.Empno))
                    .OrderBy(e => e.Ename)
